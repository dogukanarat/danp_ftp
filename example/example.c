/* example.c - CLI application demonstrating danp_ftp library */

/* All Rights Reserved */

/*
 * This example demonstrates using the danp_ftp library with:
...
static void printVersion(bool verbose)
{
    if (verbose)
    {
        const char msg[] = "Calling danp_ftp_get_version()...\n";
...
static void doAdd(int32_t a, int32_t b, bool verbose)
{
    if (verbose)
    {
        dprintf(STDERR_FILENO, "Calling danp_ftp_add(%d, %d)...\n", a, b);
    }
...
static void doMultiply(int32_t a, int32_t b, bool verbose)
{
    ...
    if (verbose)
    {
        dprintf(STDERR_FILENO, "Calling danp_ftp_multiply(%d, %d, &result)...\n", a, b);
    }
...
static void doFoo(const char *input, bool verbose)
{
    ...
    if (verbose)
    {
        dprintf(STDERR_FILENO, "Calling danp_ftp_foo(\"%s\", output, %d)...\n", input,
                BUFFER_SIZE);
    }
...
static void doBar(int32_t value, bool verbose)
{
    if (verbose)
    {
        dprintf(STDERR_FILENO, "Calling danp_ftp_bar(%d)...\n", value);
    }
...
static void doFactorial(int32_t n, bool verbose)
{
    if (verbose)
    {
        dprintf(STDERR_FILENO, "Calling danp_ftp_factorial(%d)...\n", n);
    }
    ...
    else
    {
        ...
        if (result.status == DANP_FTP_ERROR_INVALID)
        {
