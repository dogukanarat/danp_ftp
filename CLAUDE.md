...
This is **danp_ftp** - a C library template/scaffold project designed to be cloned and migrated to create new C libraries. The workflow is:
...
./test_danp_ftp       # Run test executable directly
```
...
- **include/danp_ftp/** - Public API headers (what users #include)
  - `danp_ftp.h` - Main API header
  - `danp_ftp_types.h` - Common types/definitions

- **src/** - Implementation files (not exposed to users)
  - `danp_ftp.c` - Core implementation
  - `danp_ftp_int.c` - Internal utilities (for helpers not in public API)
...
- **cmake/** - CMake modules
  - `danp_ftpConfig.cmake.in` - Package config template (enables find_package())
...
- CMake package config generation (enables `find_package(danp_ftp)`)
...
- Headers → `/usr/local/include/danp_ftp/`
- CMake config → `/usr/local/lib/cmake/danp_ftp/`
